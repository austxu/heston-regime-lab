...
def _compute_comparison(config: dict, cache: Cache, prefer_live: bool) -> dict:
    ...
    def buckets(rows: list, key: str) -> list:
        return [
            {"center": float(r[key]), "n": int(r["n"]),
             "bs": float(r["bs"]), "heston": float(r["heston"]), "corrected": float(r["corrected"])}
            for r in rows
        ]

    heston_vs_bs = 100.0 * (cmp.mae_bs - cmp.mae_heston) / cmp.mae_bs if cmp.mae_bs else 0.0
    resid_impr = 100.0 * (cmp.mae_heston - cmp.mae_corrected) / cmp.mae_heston if cmp.mae_heston else 0.0
...
def get_comparison(config: dict, cache: Cache, prefer_live: bool = True) -> dict:
    ...
    key = f"comparison:{session_suffix()}:{'live' if prefer_live else 'offline'}"
